...
class TransactionController extends Controller
{
    ...
    public function findSKU(Request $request)
    {
        ...
        $product_variant = ProductVariant::where('sku', $validated['sku'])->with('product')->first();
        if (!$product_variant) {
            return response()->json(
                [
    ...
    public function index(Request $request)
    {
        ...
        $transactions = Transaction::with(['customer', 'cashier', 'items'])
            ->when($by_search, function ($query, $by_search) {
                $query->where('invoice_code', 'like', "%{$by_search}%")->orWhereHas('customer', function ($q) use ($by_search) {
    ...
    {
        $datePart = date('Ymd');
        $countInvoiceToday = Transaction::whereDate('created_at', now()->toDateString())->count() + 1;
        return 'TRX-' . $datePart . '-' . str_pad($countInvoiceToday, 4, '0', STR_PAD_LEFT);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validated['is_new_customer'] || (isset($validated['register_customer']['name']) && isset($validated['register_customer']['phone']) && isset($validated['register_customer']['address']) && (empty($validated['register_customer']['name']) || empty($validated['register_customer']['phone']) || empty($validated['register_customer']['address'])))) {
            $validated['customer_id'] = null;
            $validated['customer_type'] = 'general';
        }
        // Simpan customer baru jika diperlukan
        if ($validated['is_new_customer'] && isset($validated['register_customer']) && isset($validated['register_customer']['name']) && isset($validated['register_customer']['phone']) && isset($validated['register_customer']['address']) && !empty($validated['register_customer']['name']) && !empty($validated['register_customer']['phone']) && !empty($validated['register_customer']['address'])) {
        ...
        foreach ($validated['items'] as $item) {
            ...
            if ($product_variant->stock < $item['qty']) {
                return response()->json(
                    [
                        'message' => 'Stok produk dengan SKU ' . $item['sku'] . ' tidak mencukupi',
                    ],
                    400,
                );
            }
    ...
    {
        $transaction = Transaction::with('customer', 'cashier', 'items.variant.product')->find($id);
        if (!$transaction) {
    ...
    public function edit($id)
    {
        ...
        if ($transaction->cashier_id != Auth::user()->id) {
            return back()->with('error', 'Transaksi ini bukan milik kasir lain');
        }
