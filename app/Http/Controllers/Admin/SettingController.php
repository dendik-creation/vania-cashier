...
class SettingController extends Controller
{
    ...
    public function update(Request $request){
        $rules = [
            ...
            'admin_fee_criteria.*.payment_method' => 'required|string|in:qris,cash,transfer',
        ...
        $setting = Setting::first();
        if ($request->hasFile('app_logo')) {
            $path = $request->file('app_logo')->storeAs('public', 'icon.' . $request->file('app_logo')->getClientOriginalExtension());
            $validated['app_logo'] = '/icon.' . $request->file('app_logo')->getClientOriginalExtension();
        } else {
